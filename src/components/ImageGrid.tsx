        ...
        >
          ...
          {isHEIC(imagem) ? (
            <img
              ...
              className={`absolute inset-0 w-full h-full object-contain ${loadingImages.has(index) ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}
            ...
            <Image
              ...
              alt={`${titulo} - Imagem ${index + 1}`}
              fill
              className={`object-contain ${loadingImages.has(index) ? "opacity-0" : "opacity-100"} transition-opacity duration-300`}
              onLoad={() => handleImageLoad(index)}
              onError={(e) => handleImageError(index, e)}
            />
          )}
